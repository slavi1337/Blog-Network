...
exports.createComment = async (req, res) => {
  ...
  if (containsCensoredWord(content, badWords)) {
    return res.status(400).json({
      error: "Vaš komentar sadrži nedozvoljene riječi.",
    });
  }
      ...
      try {
        ...
        if (parentCommentRes.rowCount > 0) {
          ...
          if (parentAuthorId !== userId) {
            await pool.query(
              `INSERT INTO notifications (recipient_id, type, related_entity_id, secondary_entity_id)
               VALUES ($1, 'reply_to_comment', $2, $3)`,
              [parentAuthorId, postId, parentCommentId]
            );
          }
        }
      } catch (notificationError) {
        console.error("Greška pri notifikacijama:", notificationError);
      }
    }
  } catch (err) {
  ...
  try {
    ...
    const permissionResult = await pool.query(
      `SELECT EXISTS (
        ...
        WHERE blogger_id = $1 AND moderator_id = $2
      )`,
